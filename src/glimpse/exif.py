...
class Exif:
    ...
    Attributes:
        tags (dict): Exif tags read from the image. The tags are grouped by Image File
            ...
            The thumbnail image, if present, is stored as :py:class:`bytes` in `thumbnail`.
        imgsz (Tuple[int, int]): Image size in pixels (nx, ny). Parsed from `PixelXDimension` and `PixelYDimension`.
        datetime (datetime.datetime): Capture date and time. Parsed from `DateTimeOriginal` and `SubSecTimeOriginal`.
        exposure (float): Exposure time in seconds. Parsed from `ExposureTime`.
        aperture (float): Aperture size as the f-number (https://wikipedia.org/wiki/F-number). Parsed from `FNumber`.
        iso (int): Film speed following the ISO system (https://wikipedia.org/wiki/Film_speed#ISO). Parsed from `ISOSpeedRatings`.
        ...
        sensorsz (Tuple[float, float]): Sensor size in millimeters (nx, ny). Data is from Digital Photography Review (https://dpreview.com) reviews and their article https://dpreview.com/articles/8095816568/sensorsizes.
    """

    def __init__(self, path: str, thumbnail: bool = False) -> None:
        self.tags: dict = piexif.load(path, key_is_name=True)
        if not thumbnail:
    ...
    def sensorsz(self) -> Optional[Tuple[float, float]]:
        make = self.make.strip() if self.make else None
        model = self.model.strip() if self.model else None
        if make and model:
            return SENSOR_SIZES.get(make + " " + model)
        return None
    ...
    def insert(self, path: str) -> None:
        ...
        Example:
            ...
            >>> exif.insert(path)
            >>> Exif(path).tags['Exif']['ISOSpeedRatings']
            100
            >>> exif.tags['Exif']['ISOSpeedRatings'] = 200
            >>> exif.insert(path)
            >>> Exif(path).tags['Exif']['ISOSpeedRatings']
            200
        """
